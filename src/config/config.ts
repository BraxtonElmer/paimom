...
export interface GenshinServer {
  name: string;
  timezone: string;
  dailyResetTime: string; // HH:mm format in local timezone
  weeklyResetDay: number; // ISO weekday: 1 = Monday, 7 = Sunday
  weeklyResetTime: string; // HH:mm format in local timezone
}
...
export interface ResinConfig {
  max: number;
  regenRate: number; // minutes per resin
}
